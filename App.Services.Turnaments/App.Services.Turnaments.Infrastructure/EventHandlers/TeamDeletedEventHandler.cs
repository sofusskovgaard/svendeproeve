...
namespace App.Services.Turnaments.Infrastructure.EventHandlers
{
    public class TeamDeletedEventHandler : IEventHandler<TeamDeletedEventMessage>
    {
        ...
        public async Task Consume(ConsumeContext<TeamDeletedEventMessage> context)
        {
            ...
            var matches = await _entityDataService.ListEntities<MatchEntity>(filter => filter.AnyStringIn(entity => entity.TeamsId, message.Id));

            foreach (var match in matches)
            {
                match.TeamsId = match.TeamsId.Where(t => t != message.Id).ToArray();

                var updateDefinition = new UpdateDefinitionBuilder<MatchEntity>().Set(entity => entity.TeamsId, match.TeamsId);

                await _entityDataService.Update<MatchEntity>(filter => filter.Eq(entity => entity.Id, match.Id), _ => updateDefinition);
            }
        }
    }
}
