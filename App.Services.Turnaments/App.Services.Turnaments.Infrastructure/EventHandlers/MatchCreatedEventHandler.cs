...
public class MatchCreatedEventHandler : IEventHandler<MatchCreatedEventMessage>
{
    ...
    public async Task Consume(ConsumeContext<MatchCreatedEventMessage> context)
    {
        ...
        var updateDefinition =
            new UpdateDefinitionBuilder<TurnamentEntity>().Set(entity => entity.MatchesId, matches.ToArray());

        await _entityDataService.Update<TurnamentEntity>(filter => filter.Eq(entity => entity.Id, message.TurnamentId),
            _ => updateDefinition);
    }
}