...
public class UpdateTeamCommandHandler : ICommandHandler<UpdateTeamCommandMessage>
{
    ...
    public async Task Consume(ConsumeContext<UpdateTeamCommandMessage> context)
    {
        ...
        if (message.Bio != team.Bio) updateDefinition = updateDefinition.Set(entity => entity.Bio, message.Bio);
        ...
        await _entityDataService.Update<TeamEntity>(filter => filter.Eq(entity => entity.Id, message.TeamId),
            _ => updateDefinition);
    }
}